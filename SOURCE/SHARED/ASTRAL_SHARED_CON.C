...
AS_U0 ASTRAL_CON_SET_COLOUR_F(ASTRAL_CON_COLOUR* COLOUR, AS_U8 R, AS_U8 G, AS_U8 B, AS_U8 A) {
    COLOUR->R_F = R;
    COLOUR->G_F = G;
    COLOUR->B_F = B;
    COLOUR->A_F = A;
}
AS_U0 ASTRAL_CON_SET_COLOUR_F_M(ASTRAL_CON_COLOUR* COLOUR, AS_U32 RGBA) {
    COLOUR->R_F = (AS_U8)(RGBA >> 24);
    COLOUR->G_F = (AS_U8)(RGBA >> 16);
    COLOUR->B_F = (AS_U8)(RGBA >> 8);
    COLOUR->A_F = (AS_U8)(RGBA);
}
AS_U0 ASTRAL_CON_SET_COLOUR_B(ASTRAL_CON_COLOUR* COLOUR, AS_U8 R, AS_U8 G, AS_U8 B, AS_U8 A) {
    COLOUR->R_B = R;
    COLOUR->G_B = G;
    COLOUR->B_B = B;
    COLOUR->A_B = A;
}
AS_U0 ASTRAL_CON_SET_COLOUR_B_M(ASTRAL_CON_COLOUR* COLOUR, AS_U32 RGBA) {
...
ASTRAL_CON_UI_ELEM *ASTRAL_CON_UI_ELEM_INIT(ASTRAL_CON_UI_ELEM* PARENT) {
    ...
    ELEMENT->ID = U64_MAX;
    ELEMENT->POS = ASTRAL_CON_CREATE_COORD(0, 0);
    ELEMENT->STYLING = ASTRAL_CON_CREATE_STYLING();
...
ASTRAL_CON_STYLING *ASTRAL_CON_CREATE_STYLING() {
    ASTRAL_CON_STYLING* STYLING = (ASTRAL_CON_STYLING*)ASTRAL_M_ALLOC(sizeof(ASTRAL_CON_STYLING));
    if(STYLING == NULLPTR) return NULLPTR;
    ASTRAL_CON_SET_COLOUR_F_M(&STYLING->COLOUR, RGBA(0, 0, 0, 0));
    // STYLING->MARGIN = ASTRAL_CON_CREATE_MARGIN(0, 0, 0, 0);
    // STYLING->PADDING = ASTRAL_CON_CREATE_PADDING(0, 0, 0, 0);
    // STYLING->BORDER_RADIUS = 0;
    // STYLING->BORDER_WIDTH = 0;
    // STYLING->BORDER_COLOUR = ASTRAL_CON_CREATE_COLOUR(0, 0, 0, 0, 0, 0, 0, 0);
    // STYLING->BORDER_STYLE = 0;
    // STYLING->SIZE = ASTRAL_CON_CREATE_SIZE(0, 0);
    return STYLING;
}
AS_U0 ASTRAL_CON_FREE_STYLING(ASTRAL_CON_STYLING* STYLING);

ASTRAL_CON_COORD *ASTRAL_CON_CREATE_COORD(AS_U64 X, AS_U64 Y) {
    ASTRAL_CON_COORD* COORD = (ASTRAL_CON_COORD*)ASTRAL_M_ALLOC(sizeof(ASTRAL_CON_COORD));
    if(COORD == NULLPTR) return NULLPTR;
    COORD->X = X;
    COORD->Y = Y;
    return COORD;
}
AS_U0 ASTRAL_CON_FREE_COORD(ASTRAL_CON_COORD* COORD) {
    ASTRAL_M_FREE(COORD);
    COORD = NULLPTR;
}
