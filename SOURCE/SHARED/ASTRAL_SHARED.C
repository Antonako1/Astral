...
AS_STRING* ASTRAL_STR_CREATE(AS_U64 SIZE, AS_CHAR* DATA, AS_U64 DATA_SIZE) {
    AS_STRING* STR = (AS_STRING*)ASTRAL_M_ALLOC(sizeof(AS_STRING));
    STR->DATA = (AS_CHAR*)ASTRAL_M_ALLOC(SIZE);
...
AS_U64 ASTRAL_STR_COPY(AS_STRING* DEST, AS_CHAR* SRC, AS_U64 SIZE) {
    if(DEST->DATA != NULLPTR) {
        ASTRAL_M_FREE(DEST->DATA);
    }
...
AS_U64 ASTRAL_STR_APPEND(AS_STRING* DEST, AS_CHAR* SRC, AS_U64 SIZE) {
    if(DEST->DATA != NULLPTR) {
        ASTRAL_M_FREE(DEST->DATA);
    }
    DEST->DATA = (AS_CHAR*)ASTRAL_M_ALLOC(SIZE);
...
AS_BOOLEAN ASTRAL_STR_CMP(AS_STRING* STR1, AS_CHAR* STR2, AS_U64 SIZE) {
    for(AS_U64 I = 0; I < SIZE; I++) {
...
AS_WSTRING* ASTRAL_WSTR_CREATE(AS_U64 SIZE, AS_WCHAR* DATA, AS_U64 DATA_SIZE);
AS_U64 ASTRAL_WSTR_COPY(AS_WSTRING* DEST, AS_WSTRING* SRC);
AS_U64 ASTRAL_WSTR_COPY(AS_WSTRING* DEST, AS_WCHAR* SRC, AS_U64 SIZE);
AS_U64 ASTRAL_WSTR_APPEND(AS_WSTRING* DEST, AS_WSTRING* SRC);
AS_U64 ASTRAL_WSTR_APPEND(AS_WSTRING* DEST, AS_WCHAR* SRC, AS_U64 SIZE);
AS_BOOLEAN ASTRAL_WSTR_CMP(AS_WSTRING* STR1, AS_WSTRING* STR2);
AS_BOOLEAN ASTRAL_WSTR_CMP(AS_WSTRING* STR1, AS_WCHAR* STR2, AS_U64 SIZE);
